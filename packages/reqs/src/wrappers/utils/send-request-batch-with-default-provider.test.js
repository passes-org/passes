...
describe('sendRequestBatchWithDefaultProvider', () => {
  it('sends an array of requests wrapped in a request-batch wrapped in a request-with-default-provider, and unwraps its result when accepted', async () => {
    ...
    const ABI_MOCK = {
      request: mock(async () => {
        ...
        const batchResult = await requestBatch.encodeResult({ status: 'accepted', body: [wrappedResult1, wrappedResult2] });
        const outerResult = await requestWithDefaultProvider.encodeResult({ status: 'accepted', body: batchResult });

        return outerResult;
      }),
    };

    const result = await sendRequestBatchWithDefaultProvider({
      requests: [
